...
class pedestrian_localizer():
    ...
    def __init__(self, calibrate_yaw=False, calibration_distance=2, yaw_method='Stable', yaw_latch=True, callback=None):
        ...
        self.logger = get_logger('pedestrian_localizer')
    ...
    def update_step_count(self, zv, imu_reading):
        """ Updates and returns current step count
            ...
            @warning: To track step count correctly, this function should be called at each time an imu reading is received 
                        (Per each iteration)
        """
        time = imu_reading.header.stamp.sec +  imu_reading.header.stamp.nanosec * 1e-9
    ...
    def calibrate(self, imu_reading, x, p, zv):
        ...
        # print(self.rp_calibrated)
        # print(len(self.calibration_readings))
        ...
        if ((not self.rp_calibrated) and (len(self.calibration_readings) > 100)):
            ...
            self.logger.info("Roll-Pitch Calibration Successfull")
            self.logger.info("IMU on foot: roll= " + str(int(roll*180/np.pi))+ "degrees     pitch= " + str(int(pitch*180/np.pi))+ "degrees")
        ...
        if ((self.rp_calibrated) and ((np.linalg.norm(x[:2]) >= self.calibration_distance) or (not self.calibrate_yaw))):
            ...
            # Calibration complete
            if self.calibrate_yaw:
                self.logger.info("Yaw Calibration Successfull")
                self.logger.info("IMU on foot: yaw= "+ str(int(self.IMU_orientation[2]*180/np.pi)) + "degrees")
            else:
                self.logger.info("Yaw Calibration disabled")
    ...
    def update_human_pose(self, x, zv):
        ...
        if self.yaw_method == 'Real':
            ...
            self.logger.error("Pedestrian_Localizer: Unknown yaw_method")
